...
const SelectionList = ({active, setActive, options, innerRef}) => {
    ...
    const handlePrev = () => {
        swiper.slidePrev();
        setActive(options[swiper.realIndex].id || options[swiper.realIndex]);
    }
    ...
    return (
        <StyledSelectionList ref={innerRef}>
            ...
            <Swiper className="selection-list w-100 h-100"
                    ...
                    slidesPerView="auto"
                    spaceBetween={12}
                    initialSlide={options.findIndex(option => option.id === active)}
                    loop={true}
                    centeredSlides={true}>
                {
                    options.map(option => (
                        <SwiperSlide key={option.name || option}>
                            <SelectionButton text={option.name || option}
                                             active={active === option.id || active === option}
                                             onClick={() => setActive(option.id || option)}/>
                        </SwiperSlide>
                    ))
                }
            </Swiper>
